...
\noindent
Doraemon is teaching Nobita a powerful searching algorithm for trees: Depth-First Search (DFS). To demonstrate, he considers the following pseudocode for finding a specific target node $U$ in a tree rooted at node $1$:
...
\noindent
Given a tree with $N$ vertices and a target node $U$, suppose Doraemon calls \texttt{DFS(1, 0)}. Because the order in which the children are visited is permuted uniformly at random at each step, the final traversal cost (the value of \texttt{cost} when the algorithm terminates) is a random variable. 
